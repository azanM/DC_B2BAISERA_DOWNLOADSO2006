...
namespace B2BAISERA.Models.EFServer
{
    public partial class CUSTOM_DOWNLOAD_TRANSACTIONDATA
    {
        ...
        public virtual ICollection<CUSTOM_S02006> CUSTOM_S02006
        {
            get
            {
                if (_cUSTOM_S02006 == null)
                {
                    var newCollection = new FixupCollection<CUSTOM_S02006>();
                    newCollection.CollectionChanged += FixupCUSTOM_S02006;
                    _cUSTOM_S02006 = newCollection;
                }
                return _cUSTOM_S02006;
            }
            set
            {
                if (!ReferenceEquals(_cUSTOM_S02006, value))
                {
                    var previousValue = _cUSTOM_S02006 as FixupCollection<CUSTOM_S02006>;
                    if (previousValue != null)
                    {
                        previousValue.CollectionChanged -= FixupCUSTOM_S02006;
                    }
                    _cUSTOM_S02006 = value;
                    var newValue = value as FixupCollection<CUSTOM_S02006>;
                    if (newValue != null)
                    {
                        newValue.CollectionChanged += FixupCUSTOM_S02006;
                    }
                }
            }
        }
        private ICollection<CUSTOM_S02006> _cUSTOM_S02006;
        ...
        private void FixupCUSTOM_S02006(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.NewItems != null)
            {
                foreach (CUSTOM_S02006 item in e.NewItems)
                {
                    item.CUSTOM_DOWNLOAD_TRANSACTIONDATA = this;
                }
            }
            ...
            if (e.OldItems != null)
            {
                foreach (CUSTOM_S02006 item in e.OldItems)
                {
                    if (ReferenceEquals(item.CUSTOM_DOWNLOAD_TRANSACTIONDATA, this))
                    {
                        item.CUSTOM_DOWNLOAD_TRANSACTIONDATA = null;
